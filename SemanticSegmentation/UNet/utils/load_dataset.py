...
from .trainer import TrainConfigs
...
def get_label_map(color_map_path: str, json_file_path: str) -> torch.Tensor:
...
class SemanticSegDataset(Dataset):
    ...
    def __getitem__(self, index):
        image = tv_tensors.Image(decode_image(self.image_paths[index], mode=ImageReadMode.RGB))
        mask = tv_tensors.Mask(get_label_map(self.colormap_paths[index], self.json_label_paths[index]))
...
def get_transform_train(input_height, input_width):
    return v2.Compose([
...
def get_transform_val(input_height, input_width):
    return v2.Compose([
        v2.Resize((input_height, input_width)),
        v2.ToDtype(torch.float32, scale=True),
        v2.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
...
def get_dataloaders(configs: TrainConfigs, num_workers: int = 8):
    ...
    transforms_train = get_transform_train(H, W)
    transforms_val = get_transform_val(H, W)
    ...
    val_dataset.dataset.transforms = transforms_val  # type: ignore
