...
# 1. Load Session
session = ort.InferenceSession("exported_models/inference_model.onnx", providers=['CPUExecutionProvider'])
input_name = session.get_inputs()[0].name

# 2. Preprocess Image
img_orig = cv2.imread("/home/linde/Downloads/rf/valid/Replicator/0007.png")
h_orig, w_orig = img_orig.shape[:2]
img = cv2.cvtColor(img_orig, cv2.COLOR_BGR2RGB)
img = cv2.resize(img, (504, 504)) # Typical RT-DETR input size
...
print(boxes.shape, scores.shape, mask_logits.shape)
...
def visualize(image, boxes, scores, masks, threshold=0.5):
    ...
    for i in range(len(boxes[0])):
        ...
        cv2.addWeighted(annotated, 1.0, colored_mask, 0.5, 0, annotated)
