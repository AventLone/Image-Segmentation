...
from .ms_deform_atten import _bilinear_grid_sample
...
class MLPBlock(nn.Module):
    def __init__(self, dim, layer_scale_init_value=0):
        ...
        self.layers = nn.ModuleList([nn.Linear(dim, dim * 4),
                                     nn.GELU(),
                                     nn.Linear(dim * 4, dim)])
...
class SegmentationHead(nn.Module):
    ...
    def forward(self, spatial_features: torch.Tensor,
                ...
                skip_blocks: bool = False) -> list[torch.Tensor]:
...
def get_uncertain_point_coords_with_randomness(
    ...
    importance_sample_ratio: float = 0.75) -> torch.Tensor:
    """
